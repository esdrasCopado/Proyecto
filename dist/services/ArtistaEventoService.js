...
class ArtistaEventoService {
     ...
     */
    async validarArtistaYEvento(artistaId, eventoId) {
    ...
    async asignarArtistaAEvento(datos) {
        ...
        const artistaEvento = new ArtistaEvento_1.ArtistaEvento({
            artistaId: datos.artistaId,
            eventoId: datos.eventoId,
            rol: datos.rol || enums_1.RolArtista.INVITADO,
     ...
     */
    async obtenerArtistasDeEvento(eventoId) {
        if (eventoId <= 0) {
     ...
     */
    async obtenerEventosDeArtista(artistaId) {
        if (artistaId <= 0) {
     ...
     */
    async obtenerHeadlinersDeEvento(eventoId) {
        if (eventoId <= 0) {
            throw new Error('El ID del evento debe ser válido');
        }
        // Verificar que el evento exista
        ...
        const headliners = await this.artistaEventoRepository.findByEventoIdAndRol(eventoId, enums_1.RolArtista.HEADLINER);
        return headliners.map(asignacion => asignacion.toJSON());
    }
    /**
     * Actualiza el rol y/o compensación de un artista en un evento
     */
    async actualizarRolYCompensacion(artistaId, eventoId, datos) {
        // Validar IDs
     ...
     */
    async removerArtistaDeEvento(artistaId, eventoId) {
        // Validar IDs
        await this.validarArtistaYEvento(artistaId, eventoId);
        // Buscar la asignación existente
        const asignaciones = await this.artistaEventoRepository.findByEventoId(eventoId);
     ...
     */
    async obtenerArtistasPorRol(eventoId, rol) {
        if (eventoId <= 0) {
            throw new Error('El ID del evento debe ser válido');
        }
...
exports.ArtistaEventoService = ArtistaEventoService;
//# sourceMappingURL=ArtistaEventoService.js.map