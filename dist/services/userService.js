...
class UserService {
     ...
     */
    async searchUsersByName(nombre) {
        try {
     ...
     */
    async getUsersByDateRange(fechaInicio, fechaFin) {
        try {
     ...
     */
    async getUsersWithPagination(page, pageSize) {
        try {
            if (page <= 0) {
    ...
    async createUser(userData) {
        try {
            ...
            const usuario = new Usuario_1.Usuario({
                ...
                rol: userData.rol || enums_1.Role.USER,
    ...
    async updateUser(id, updateData) {
        try {
            ...
            if (updateData.password) {
                updateData.password = await (0, bcrypt_1.hashPassword)(updateData.password);
            }
    ...
    async getUserStats() {
        try {
            ...
            const usuarios = await this.usuarioRepository.findByRol(enums_1.Role.USER);
            const artistas = await this.usuarioRepository.findByRol(enums_1.Role.ARTISTA);
            const organizadores = await this.usuarioRepository.findByRol(enums_1.Role.ORGANIZADOR);
            const admins = await this.usuarioRepository.findByRol(enums_1.Role.ADMIN);
    ...
    async validateCredentials(email, password) {
        try {
            ...
            const isPasswordValid = await (0, bcrypt_1.comparePassword)(password, usuario.password);
            if (isPasswordValid) {
                return usuario;
            }
            return null;
