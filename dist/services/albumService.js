...
class AlbumService {
     ...
     */
    async obtenerAlbumsPorArtista(artistaId) {
        if (artistaId <= 0) {
     ...
     */
    async obtenerAlbumsPorGenero(genero) {
     ...
     * Actualiza un álbum
     */
    async actualizarAlbum(id, datosActualizar) {
        if (id <= 0) {
            throw new Error('El ID del álbum debe ser válido');
        }
        ...
        if (datosActualizar.lanzamiento !== undefined) {
            const fecha = typeof datosActualizar.lanzamiento === 'string'
     ...
     */
    async contarAlbumsPorArtista(artistaId) {
        if (artistaId <= 0) {
            throw new Error('El ID del artista debe ser válido');
        }
