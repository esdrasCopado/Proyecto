...
const OrdeneService_1 = require("../services/OrdeneService");
/**
...
class OrdenController {
    constructor() {
         ...
         */
        this.crearOrden = async (req, res) => {
            try {
                const { usuarioId, boletoIds } = req.body;
                // Validación básica
                if (!usuarioId || !boletoIds) {
                    return this.errorResponse(res, new Error('Faltan campos requeridos: usuarioId, boletoIds'), 400);
                }
                if (!Array.isArray(boletoIds) || boletoIds.length === 0) {
                    return this.errorResponse(res, new Error('boletoIds debe ser un array con al menos un elemento'), 400);
                }
         ...
         */
        this.obtenerOrdenPorId = async (req, res) => {
            try {
         ...
         */
        this.obtenerTodasLasOrdenes = async (req, res) => {
            try {
         ...
         */
        this.obtenerMisOrdenes = async (req, res) => {
            try {
         ...
         */
        this.obtenerOrdenesPorUsuario = async (req, res) => {
            try {
         ...
         */
        this.obtenerOrdenesPorEstado = async (req, res) => {
            try {
         ...
         */
        this.actualizarEstadoOrden = async (req, res) => {
            try {
                const ordenId = this.validarNumero(req.params.id, 'ID de la orden');
                const { estado } = req.body;
                if (!estado) {
                    return this.errorResponse(res, new Error('El campo estado es requerido'), 400);
                }
                const ordenActualizada = await this.ordenService.actualizarEstadoOrden(ordenId, estado.toUpperCase());
         ...
         */
        this.marcarComoPagada = async (req, res) => {
            try {
                const ordenId = this.validarNumero(req.params.id, 'ID de la orden');
         ...
         * Cancela una orden
         */
        this.cancelarOrden = async (req, res) => {
            try {
                const ordenId = this.validarNumero(req.params.id, 'ID de la orden');
         ...
         */
        this.reembolsarOrden = async (req, res) => {
            try {
                const ordenId = this.validarNumero(req.params.id, 'ID de la orden');
         ...
         * Elimina una orden
         */
        this.eliminarOrden = async (req, res) => {
            try {
                const ordenId = this.validarNumero(req.params.id, 'ID de la orden');
         ...
         */
        this.obtenerEstadisticas = async (req, res) => {
            try {
        ...
        this.ordenService = new OrdeneService_1.OrdenService();
    }
    /**
